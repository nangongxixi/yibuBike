...
class Bike extends Base
{
    ...
    public function __construct(\Windward\Core\Container $container)
    {
        ...
        $this->model = $container->model('Bike');
    }

    public function indexAction()
    {
        $sh = $this->request->getQuery('sh');
        $userlist = $this->model->bikeList($sh);
    ...
    public function bookLogAction()
    {
        $sh = $this->request->getQuery('sh');
        $userlist = $this->model->booklog($sh);
        $response = new \Windward\Core\Response\Smarty($this->container);
        $response->set('results', $userlist);
        $response->set('sh', $sh);
        return $response;
    ...
    public function troubleLogAction() //故障处理记录
    {
        $sh = $this->request->getQuery('sh');
        $userlist = $this->model->booklog($sh);
        $response = new \Windward\Core\Response\Smarty($this->container);
